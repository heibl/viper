...
#' @importFrom graphics lines par rect strheight strwidth text
...
axisGeo <- function(GTS, tip.time = 0, unit = c("epoch", "period"), ages =
  ...
  ## -----------
  if ( lastPP$direction == "rightwards" ){
    if ( ages ) yy <- c(bh, 2 * bh) + .6 else yy <- c(0, bh)
...
#     if (ages) yy <- c(bh, 2*bh) else yy <- c(0, bh)
  ...
  if ( ages ) {
    ...
    label <- label[id]
    if (lastPP$direction == "rightwards")
      text(xgrid, 0.5, round(label, digits = 1), cex = cex * 0.8)
      text(max(xgrid, na.rm = TRUE) * 1.1, 0.5, "Ma", cex = cex * 0.8)
    if (lastPP$direction == "upwards")
