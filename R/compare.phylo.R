...
#' @importFrom ape is.ultrametric plot.phylo
...
compare.phylo <- function(x, y, ts = "arrow", threshold, presCol = "green", absCol = 
	...
	cat("\n", paste(length(missingINy), 
		...
		paste0(missingINy, collapse = "\n\t"), sep = ""))
	missingINx <- setdiff(y$tip.label, x$tip.label)
	cat(paste("\n", length(missingINx), 
		" tips from 'y' are missing in 'x'\n\t", 				
		paste0(missingINx, collapse = "\n\t"), "\n"))
	if (length(missingINy))
		x <- drop.tip2(x, missingINy)
	if (length(missingINx))
		y <- drop.tip(y, missingINx)
	
	# plot phylogeny 'x'
	# ------------------										
	if (plottype == "visible") plot.phylo(x, cex = tipcex, ...)
	if (plottype == "invisible") plot.phylo(x, tip.color = 0, edge.color = 0,
	     cex = tipcex, ...)
	if (plottype == "upon") plotPhyloUpon(x, cex = tipcex, ...)
	...
	# -----------------------------------------------
	set <- lapply(xnodes, descendants, phy = x, type = "t", labels = TRUE)
